		...
		<div class="row  <?= strtolower($table_name) ?>" style="margin-top: 25px">
			<div class="row" style="margin-bottom: 20px">
				<div class="col_6">
					...
					<input class="form-element required" type="text" placeholder="Location Status" id="location_status" value="<?= ($action === "edit")? $data["location_status"] : "" ?>">
				</div>	
			</div>
			<div class="row" style="margin-bottom: 20px">
				<div class="col_6">
			...
			<div class="row" style="margin-bottom: 20px">
				<div class="col_1">
					...
					<select id="color" class="form-element">
						<option selected value="-1"></option>
							<?php  
								foreach( $colors as $k=>$v){
							?>	
						<option data-hex='<?= $v["rgba"] ?>' data-id='<?= $v["id"] ?>' style='color:white;background-color:<?= $v["hex"] ?>' value="<?= $v["rgba"] ?>"> <?= $v["name"] ?> </option>
							<?php } ?>
					</select>
				</div>	
				<div class="col_1">
					<label for="">.</label>
					<div id="color_display" style="background-color: white; padding:16px 10px"></div>
				</div>
				<div class="col_1 hide">
					<button class="btn btn-default color_converter">convert</button>
				</div>
			</div>
