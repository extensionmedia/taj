...
<div class="panel">
	...
	<div class="panel-header" style="padding: 0px">
		<div class="panel-header-tab ">
			...
			<a  href="" class="show_files category" data="<?= ($action === "edit")? $data["UID"] : substr($formToken,0,8) ?>"><i class="fas fa-images"></i> Images</a>
		</div>
	</div>
			...
			<?= ($action === "edit")? "<input class='form-element' type='hidden' id='id' value='".$id."'>" : "" ?>
				...
				<div class="row" style="margin-bottom: 20px">
					<div class="col_6">
						...
						<input class="form-element required" type="text" placeholder="Catégorie" id="produit_category" value="<?= ($action === "edit")? $data["produit_category"] : "" ?>">
					</div>	
					<div class="col_6-inline">
						...
						<select id="id_parent" class="form-element">
							...
							<?php 
								...
								foreach($produit_category->find(null, array("conditions"=>array("status="=>1), "order"=>"produit_category"), null) as $k=>$v){
							?>
							<option <?= ($action === "edit")? ($v["id"] === $data["id_parent"])? "selected" : "" : "" ?>  value="<?= $v["id"] ?>"> <?= $v["produit_category"] ?> </option>
							<?php } ?>
						</select>
					</div>	
				</div>
