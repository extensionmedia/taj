<form id="idSearch" method="GET" action="{{route('produit.search')}}" class="flex items-center justify-between gap-4 py-1 bg-white bg-opacity-0">
    ...
    <div class="flex items-center gap-4">
        <select name="category" id="category" class="submit_me border rounded py-2 px-2 text-xs">
        ...
        <select name="marque" id="marque" class="submit_me border rounded py-2 px-2 text-xs">
            <option value="-1">-- Marques</option>
            @foreach ($produit_marques as $m)
                <option value="{{$m->id}}">{{ Str::upper( $m->produit_marque )}}</option>
            @endforeach
        </select>
        <select name="magasin" id="magasin" class="submit_me border rounded py-2 px-2 text-xs">
